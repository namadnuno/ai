...
export class SearchFilesHandler {
  ...
  handle(args) {
    ...
    if (!args.pattern) {
      return { content: [{ type: "text", text: "error: pattern required" }], isError: true };
    }
    syncIndex(this.ctx.ROOT, this.ctx.db);
    const matches = getAllPaths(this.ctx.db).filter((p) => minimatch(p, args.pattern, { matchBase: false }));
